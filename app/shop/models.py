...
class Customer(models.Model):
    ...
    username = models.CharField(
        "Username", max_length=200, blank=True, null=True, default=None
    )
    customer_name = models.CharField(
        "ФІО користувача", max_length=255, blank=True, default=""
    )
    phone_number = models.CharField(
        "Номер телефону", max_length=64, blank=True, default=""
    )
    city = models.CharField("Місто", max_length=200, blank=True, default="")
    address = models.CharField(
        "Адреса доставки", max_length=255, blank=True, default=""
    )
    post_number = models.CharField(
        "Номер відділення", max_length=32, blank=True, default=""
    )
    date_joined = models.DateTimeField("Дата реєстрації", auto_now_add=True)
    state = models.CharField(
        max_length=200, blank=True, null=True, default=None, editable=False
    )
...
class Cart(models.Model):
    ...
    def get_subtotal(self):
        active_items = CartItem.objects.filter(
            cart=self.pk, is_active=True, product__quantity__gt=0
        )
        total_price = 0
        quantity = 0
...
class Order(models.Model):
    ...
    post_number = models.CharField(
        "Номер відділення", blank=True, max_length=32, default=""
    )
...
class OrderItem(models.Model):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.SET_NULL, verbose_name="Товар", null=True
    )
    ...
    def __str__(self):
        return (
            f"{self.product.title} | Кількість: {self.quantity} | "
            f"Сумма: {self.total}"
        )
