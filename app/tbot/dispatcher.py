...
def show_catalog(obj, bot, page_num=1):
    ...
    for item in items_per_page:
        keyboard.add(types.InlineKeyboardButton(
            text=item.name,
            callback_data=f'product_list|{item.pk}'))

    if items_per_page.has_previous():
        next_page = items_per_page.previous_page_number()
        keyboard_page_btn.append(types.InlineKeyboardButton(
            text='⬅️',
            callback_data=f'category_list|{next_page}'))
    ...
    if page_num == 1:
        bot.send_message(obj.chat.id,
                         'Оберіть категорію яка Вас цікавить:',
                         reply_markup=keyboard)
    else:
        bot.edit_message_reply_markup(obj.from_user.id,
                                      obj.message.message_id,
                                      reply_markup=keyboard)


def show_products_list(obj, bot, category_id, page_num=1):
    products_stock = Product.objects.filter(category=category_id,
                                            quantity__gt=0,
                                            is_active=True).distinct()
    products_out_of_stock = Product.objects.filter(category=category_id,
                                                   quantity__lte=0,
                                                   is_active=True).distinct()
    products = list(
        sorted(
    ...
    if not products:
        bot.send_message(obj.from_user.id,
                         'Нажаль, активних товарів в цій категорії немає.')
    ...
    for index, product in enumerate(products_per_page, start=1):
        ...
        keyboard.add(types.InlineKeyboardButton(
            text='ℹ️ Подробиці',
            callback_data=f'product_item|{product.pk}'))
        ...
        if product.image:
            bot.send_photo(obj.from_user.id,
                           product.image,
                           product_text,
                           reply_markup=keyboard)
        else:
    ...
    if not cart_item_new:
        if cart_item.quantity >= product.quantity:
            bot.answer_callback_query(obj.id,
                                      'Нажаль, поки це все що є в наявності.',
                                      False)
        else:
...
def add_one_more_item(obj, bot, item_id, is_cart=False):
    ...
    if cart_item.quantity >= cart_item.product.quantity:
        ...
        bot.answer_callback_query(obj.id,
                                  f'Додано 1 одиницю товару до корзини.',
                                  show_alert=False)
...
def remove_one_item(obj, bot, item_id, is_cart=False):
    ...
    if cart_item.quantity <= 1:
        ...
        get_item_text_and_keyboard(obj, bot, cart_item, is_cart)
        bot.answer_callback_query(obj.id,
                                  f'Видалено 1 одиницю товару з корзини.',
                                  show_alert=False)
...
def show_cart(obj, bot):
    ...
    cart_total_obj = bot.send_message(chat_id=obj.from_user.id,
...
def new_order_delivery(obj, bot, confirmed=False):
    ...
    if user.city and user.address:
        ...
        if user.city:
            ...
            keyboard.add(types.InlineKeyboardButton(
                'Пошук',
                switch_inline_query_current_chat=user.city))
...
def new_order_finish(obj, bot, confirmed=False, from_cart=False):
    ...
    keyboard = types.InlineKeyboardMarkup(row_width=2)
    keyboard.add(types.InlineKeyboardButton('Підтвердити',
                                            callback_data='confirm_order'),
                 types.InlineKeyboardButton('Змінити',
...
def search_city(search, query, bot):
    ...
    for result in cities:
        ...
        if result['RegionsDescription']:
            description = f'{result["RegionsDescription"]}, ' \
